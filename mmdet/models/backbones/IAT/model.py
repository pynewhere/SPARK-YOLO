...
class IAT(nn.Module):
    ...
    def forward(self, img_low):
        ...
        if not self.with_global:
            ...
            for i in range(b):
                image = img_high[i, :, :, :]
                color_matrix = color[i, :, :]
                gamma_value = gamma[i, :]
                processed_image = self.apply_color(image, color_matrix) ** gamma_value
                processed_images.append(processed_image)
            img_high = torch.stack(processed_images, dim=0)
            '''
            img_high = img_high.permute(0, 3, 1, 2)  # (B,H,W,C) -- (B,C,H,W)
            #print(img_high.shape)
            transforms.ToPILImage()(img_high[0].squeeze(0)).save('IAT_out.jpg')
            return mul, add, img_high
